...
Descripción:
Script de ejecución estricta para el motor de parseo OpenDataLoader.
...
Acciones Principales:
    - Detección heurística multicapa de generación de PDF con LaTeX.
    - Carga e inicialización del motor OpenDataLoaderParser.
    - Conversión del PDF a Markdown preservando el layout y las tablas.
    - Almacenamiento del Markdown enriquecido en la carpeta de fixtures.
    - Reporte detallado de tiempos, metadatos y rendimiento.
...
Entradas / Dependencias:
    ...
    - El entorno de ejecución de Java 11+ en el sistema.
    - Las dependencias de Python: `opendataloader-pdf`, `pypdf`, `pyyaml`.

Salidas / Efectos:
    - Genera un archivo Markdown enriquecido en `tests/backend/integration/fixtures/expected_outputs/`.

Ejecución:
    uv run python tests/temporal/run_opendataloader.py [--pdf PATH_TO_PDF]

Ejemplo de Uso:
    uv run python tests/temporal/run_opendataloader.py --pdf backend/data/projects/Variables_derivadas_de_imagenes_espectrales/Busqueda_1/descargas/2020_Casal_Classifying_sleep-wake_stages_through_recurrent_ne.pdf
...
from app.services.document_parser_service import OpenDataLoaderParser, OPENDATALOADER_AVAILABLE
...
async def main():
    """
    Punto de entrada principal para el script de OpenDataLoader.
    """
    parser = argparse.ArgumentParser(description="Parser estricto OpenDataLoader.")
    parser.add_argument("--pdf", type=str, help="Ruta al PDF a procesar.")
    args = parser.parse_args()
    
    # 1. Validar disponibilidad de OpenDataLoader
    if not OPENDATALOADER_AVAILABLE:
        print("[ERROR] OpenDataLoader no está instalado o no es accesible.")
        print("Por favor, asegúrate de tener instalado Java 11+ y el paquete 'opendataloader-pdf'.")
        sys.exit(1)
    ...
    print(f"\n[INICIO] Iniciando procesamiento estricto con OpenDataLoader")
    ...
    mock_meta = {
        "id": f"odl-test-{pdf_path.stem[:15]}",
        "doi": "10.48550/mock.doi",
        "title": pdf_path.stem.replace("_", " "),
        "authors": "Casal et al. / Test Runner",
        "year": 2020,
        "journal": "AgriSearch Integration Test",
        "keywords": ["spectral images", "latex comparison", "opendataloader"],
        "source_database": "arxiv",
    }
    
    # Agregar información de LaTeX a los metadatos para persistencia en RAG
    ...
    print("\n[PASO 2] Convirtiendo PDF a Markdown con OpenDataLoaderParser...")
    odl = OpenDataLoaderParser(hybrid_mode=False)
    
    start_time = time.perf_counter()
    try:
        markdown_output = await odl.parse_pdf(
            pdf_path=pdf_path,
    ...
    # 6. Almacenar el archivo resultante en las fixtures de integración
    ...
    output_file = fixtures_outputs_dir / f"{pdf_path.stem}_opendataloader.md"
    ...
    print("\n[FIN] Ejecución de OpenDataLoader finalizada correctamente.\n")
