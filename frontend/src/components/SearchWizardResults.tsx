                    ...
                    <tbody className="divide-y divide-slate-700/30">
                        {sortedArticles.map((a) => (
                            <React.Fragment key={a.id}>
                                <tr className="group hover:bg-slate-800/40 transition-all duration-200">
                                            ...
                                            <div className="p-6 transition-all duration-300 animate-in fade-in slide-in-from-top-4">
                                                <div className="max-w-6xl mx-auto space-y-4">
                                                    ...
                                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                        <div className="space-y-4 bg-slate-900/60 p-4 rounded-xl border border-slate-700/50">
                                                            <div>
                                                                <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Resumen Proporcionado por IA</span>
                                                                <div className="mt-2 text-slate-200 leading-relaxed text-sm italic">
                                                                    <Latex>{a.llm_summary || a.abstract || "Sin resumen disponible."}</Latex>
                                                                </div>
                                                            </div>
                                                            {a.methodology_type && (
                                                                <div>
                                                                    <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Metodología Detectada</span>
                                                        ...
                                                        <div className="space-y-4 bg-slate-900/60 p-4 rounded-xl border border-slate-700/50">
                                                            <div>
                                                                <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Variables Agrícolas Extraídas</span>
                                                                <div className="mt-3 flex flex-wrap gap-2">
                                                                    {a.agri_variables_json ? (
                                                                        (() => {
                                                                            try {
                                                                                const vars = JSON.parse(a.agri_variables_json);
                                                                                return Object.entries(vars).map(([key, value]) => (
                                                                                    <div key={key} className="flex flex-col gap-1 w-full p-2 bg-slate-800/50 rounded-lg border border-slate-700/30">
                                                                                        <span className="text-[10px] text-indigo-400 font-bold uppercase">{key}</span>
                                                                                        <span className="text-xs text-slate-300">{Array.isArray(value) ? value.join(", ") : String(value)}</span>
                                                                                    </div>
                                                                                ));
                                                                            } catch (e) {
                                                                                return <span className="text-xs text-slate-500">Error al procesar variables.</span>
                                                                            }
                                                                        })()
                                                                    ) : (
                                                                        <span className="text-xs text-slate-500">No se detectaron variables específicas.</span>
                                                                    )}
                                                                </div>
                                                            </div>
                                                            
                                                            <div className="flex items-center justify-between pt-2 border-t border-slate-700/50">
                                                                <span className="text-xs font-bold text-slate-500 uppercase">Puntuación Relevancia</span>
                                                                <div className="flex items-center gap-2">
                                                                    <div className="w-24 h-2 bg-slate-700 rounded-full overflow-hidden">
                                                                        <div 
                                                                            className="h-full bg-gradient-to-r from-indigo-500 to-purple-500" 
                                                                            style={{ width: `${(a.relevance_score || 0) * 100}%` }}
                                                                        ></div>
                                                                    </div>
                                                                    <span className="text-sm font-bold text-indigo-400">{((a.relevance_score || 0) * 100).toFixed(0)}%</span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="flex justify-end gap-3 pt-2">
                                                        {a.local_pdf_path && (
                                                            <button 
                                                                onClick={() => window.open(`http://localhost:8000/api/v1/screening/sessions/${a.id}/pdf`, '_blank')}
                                                                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-xs font-bold transition-all flex items-center gap-2 shadow-lg shadow-indigo-900/20"
                                                            >
                                                        ...
                                                        <button 
                                                            onClick={() => navigator.clipboard.writeText(a.abstract || "")}
