...
export default function ProjectDashboard() {
    ...
    useEffect(() => {
        ...
        getProject(id).then(async (p) => {
            ...
            setProjectDesc(p.description || null);
            try {
            ...
            <div className="max-w-5xl">
                <div className="flex items-center justify-between mb-8">
                    <div>
                        ...
                        {projectDesc ? (
                            <p className="text-slate-400 whitespace-pre-wrap">{projectDesc}</p>
                        ) : (
                            <p className="text-slate-500 italic">Sin descripción proporcionada.</p>
                        )}
                    </div>
                    <a
                        href={`/search?id=${projectId}`}
                        className="px-5 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white font-medium rounded-xl shadow-lg shadow-emerald-500/25 transition-all flex items-center gap-2 flex-shrink-0 ml-4"
                    >
