interface Props {
  ...
  screeningStatus: "included" | "maybe" | "all";
  layout: "hierarchical" | "force" | "circular";
  onThresholdChange: (t: number) => void;
  onStatusFilterChange: (s: "all" | "included" | "cited_external") => void;
  onScreeningStatusChange: (s: "included" | "maybe" | "all") => void;
...
export default function GraphToolbar({
  graphType, threshold, statusFilter, screeningStatus, layout,
  onThresholdChange, onStatusFilterChange, onScreeningStatusChange, onLayoutChange, onGraphTypeChange,
  nodeCount, edgeCount, hasScreeningData
}: Props) {
  return (
    <div className="mb-4 bg-slate-900/40 rounded-xl border border-slate-800 p-4">
      <div className="flex flex-wrap items-center gap-4">
        ...
        {/* Screening Status Selector */}
        {hasScreeningData && (
          <div className="flex items-center gap-2">
            <span className="text-[10px] uppercase tracking-[0.15em] text-slate-500 font-bold">Screening</span>
            <select
              value={screeningStatus}
              onChange={(e) => onScreeningStatusChange(e.target.value as "included" | "maybe" | "all")}
